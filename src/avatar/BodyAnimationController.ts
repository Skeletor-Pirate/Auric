...
export class BodyAnimationController {
  ...
  private applySeatedPose(): void {
    ...
    // Left arm: resting forward on lap/desk (hands down, not raised)
    const leftShoulder = b(BONES.leftShoulder);
    const leftArm = b(BONES.leftArm);
    const leftForeArm = b(BONES.leftForeArm);
    const leftHand = b(BONES.leftHand);
    if (leftShoulder) leftShoulder.rotation.set(0, 0, 0);
    if (leftArm) leftArm.rotation.set(0.5, 0.2, 2.7);
    if (leftForeArm) leftForeArm.rotation.set(-1.2, -0.15, 0.1);
    if (leftHand) leftHand.rotation.set(0.2, 0.0, 0.1);

    // Right arm: relaxed, symmetric with left (hands down)
    ...
    if (rightArm) rightArm.rotation.set(0.5, -0.2, -2.7);
    if (rightForeArm) rightForeArm.rotation.set(-1.2, 0.15, -0.1);
    if (rightHand) rightHand.rotation.set(0.2, 0.0, -0.1);
